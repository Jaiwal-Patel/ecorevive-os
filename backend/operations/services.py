...
# Residents receive request-status notifications only for these milestones.
REQUEST_NOTIFICATION_STATUSES = {
    ...
    *,
    ...
    if to_status not in ALLOWED_TRANSITIONS[current]:
        raise ValidationError(
            f"Invalid transition from {current} to {to_status}."
        )
    ...
    record_event(
        ...
        event_type="collection.status_transition",
        summary=(
            f"{request_obj.public_reference}: "
            f"{current} → {to_status}"
        ),
        object_type="CollectionRequest",
        object_id=request_obj.id,
    ...
    if to_status in REQUEST_NOTIFICATION_STATUSES:
        transaction.on_commit(
...
def _queue_notification_safely(
    request_id,
    previous_status,
    new_status,
):
